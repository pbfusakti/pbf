...
class App_Model_General_DbTable_CourseGroup extends Zend_Db_Table_Abstract {
	...
	public function getInfoSchedulle($idGroup){
		...
		$select = $db ->select()
		->from(array('cg'=>$this->_name))
		->join(array('sc'=>'course_group_schedule'),'cg.IdCourseTaggingGroup=sc.idGroup')
		->where('IdCourseTaggingGroup = ?',$idGroup);
		$row = $db->fetchAll($select);
	...
	public function getInfoSchedulleByLecturer($idsemester,$idlec=null){
		...
		->from(array('cg'=>$this->_name))
		->join(array('sc'=>'course_group_schedule'),'cg.IdCourseTaggingGroup=sc.idGroup')
		->join(array('sm'=>'tbl_subjectmaster'),'sm.IdSubject=cg.IdSubject',array('subject_code'=>'SubCode','subject_name'=>'subjectMainDefaultLanguage','faculty_id'=>'IdFaculty'))
		->join(array('s'=>'tbl_semestermaster'),'s.IdSemesterMaster=cg.IdSemester',array('semester_name'=>'SemesterMainName'))	  
	...
	public function getInfoGrroupALl($idGroup,$idprogram=null,$idsem=null){
		...
		$select = $db ->select()
		->distinct()
		->from(array('srs'=>"tbl_studentregsubjects"),array('IdStudentRegistration'))
		->join(array('tg'=>'tbl_course_tagging_group'),'tg.IdCourseTaggingGroup=srs.IdCourseTaggingGroup',array())
	...
	public function getMarkApprovalGroupList($subject_id,$idSemester,$program_id=null) {
		...
		$select = $db ->select()
		 ...
		 if ($program_id!='') {
		 	$select->join(array('pr'=>'course_group_program'),'pr.group_id=cg.idcoursetagginggroup');
	...
	public function getCourseRegisterByStudent($idSemester,$idstd){
		...
		$select = $db ->select()
		->from(array('cg'=>$this->_name))
		->join(array("srs"=>'tbl_studentregsubjects'),'srs.IdCourseTaggingGroup=cg.IdCourseTaggingGroup')
		->join(array('cgs'=>'course_group_schedule'),'cgs.idGroup=cg.IdCourseTaggingGroup')
		->join(array('s'=>'tbl_semestermaster'),'s.IdSemesterMaster=cg.IdSemester',array('semester_name'=>'SemesterMainName'))
		->join(array('pr'=>'course_group_program'),'pr.group_id=cg.IdCourseTaggingGroup')
		->joinLeft(array('stm'=>'tbl_staffmaster'),'stm.IdStaff=cg.IdLecturer',array('FrontSalutation','coordinator'=>'FullName','BackSalutation'))
		...
		if ($idSemester!=null) $select->where('s.IdSemesterMaster=?',$idSemester);
		if ($idstd!=null) $select->where('srs.IdStudentRegistration=?',$idstd);
    ...
    public function getMarkEntryGroupListt($idSubject,$idSemester){
		...
		$select = $db ->select()
					  ->from(array('cg'=>$this->_name))
					  ->joinLeft(array('s'=>'tbl_semestermaster'),'s.IdSemesterMaster=cg.IdSemester',array('semester_name'=>'SemesterMainName'))
					  ->joinLeft(array('stm'=>'tbl_staffmaster'),'stm.IdStaff=cg.IdLecturer',array('FrontSalutation','FullName','BackSalutation','StaffId'))
					 // ->joinLeft(array('sm'=>'tbl_subjectmaster'),'sm.IdSubject=cg.IdSubject',array('subject_code'=>'SubCode','subject_name'=>'subjectMainDefaultLanguage'))
					  ->where('IdSubject = ?',$idSubject)
					  ->where('IdSemester = ?',$idSemester);	
	...
	public function isCourseGroupEmpty($idGroup) {
		...
		->from(array('a'=>$this->_name),array())
		 ->join(array('srs'=>'tbl_studentregsubjects'),'a.IdCourseTaggingGroup=srs.IdCourseTaggingGroup',array('IdStudentRegistration'))
		->where('a.IdCourseTaggingGroup=?',$idGroup);
		$row = $db->fetchAll($select);
