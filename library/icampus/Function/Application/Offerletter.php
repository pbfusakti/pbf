...
class icampus_Function_Application_Offerletter extends Zend_View_Helper_Abstract{
	...
	public function generateOfferLetter($txnId){
		...
		if(!$this->islocalNationality($txnId)){
			...
			$feeStructureData = $feeStructureDb->getApplicantFeeStructure($intakeData[0]['IdIntake'],$program[0]["program_id"],null,315);	
			$biaya = $biaya*2;
			$biaya = number_format($biaya, 2, '.', ',');
		}else{
			//default to local
			$feeStructureData = $feeStructureDb->getApplicantFeeStructure($intakeData[0]['IdIntake'],$program[0]["program_id"]);
			$biaya = number_format($biaya, 2, '.', ',');
		}
		...
		*/
		...
		if ($applicant['appl_nationality']=='121' && $program[0]['program_code']=='0400' ) 
			$html_template_path = DOCUMENT_PATH."/template/OfferLetterMalay.html";
	...
	public function generateCTOfferLetter($txnId){
		$translate = Zend_Registry::get('Zend_Translate');
		...
		*/
		...
		$html_template_path = DOCUMENT_PATH."/template/CTOfferLetter.html";
		...
		//output filename
		$output_filename = $txnData["at_pes_id"]."_CT_offer_letter.pdf";
	...
	public function generateUsmOfferLetter($txnId){
		...
		if(!$this->islocalNationality($txnId)){
			//315 is foreigner in lookup db
			$feeStructureData = $feeStructureDb->getApplicantFeeStructure($intakeData[0]['IdIntake'],$program["program_id"],null,315);	
			$biaya = $biaya*2;
			$biaya = number_format($biaya, 2, '.', ',');
		}else{
			//default to local
			$feeStructureData = $feeStructureDb->getApplicantFeeStructure($intakeData[0]['IdIntake'],$program["program_id"]);
			$biaya = number_format($biaya, 2, '.', ',');
		}
