<?php
class App_Model_General_DbTable_LandscapeBlockSubject extends Zend_Db_Table_Abstract
{
	...
	public function getLandscapeBlockCourseList($idLandscape,$idBlock,$idSemester,$offer="Yes"){
    	  ...
    	  $select = $db->select()
		 				 ...
		 				 ->order("s.BahasaIndonesia");
		if($offer=="Yes"){
			$select->where("bs.subjectid IN (?)",$select_course_offer);
		}
...
public function getBlockCourseInfo($idLandscape,$idBlock,$subjectId){
   ...
   public function getBlockId($idLandscape,$subjectid){
    ...
    public function getAllParent($IdLandscape){
    	...
    	->from(array("lbs"=>"tbl_landscapeblocksubject"))
    	->join(array('sm'=>'tbl_subjectmaster'),'sm.IdSubject=lbs.subjectid',array('CreditHours','SubCode','SubjectName','NamaSubjek'=>'BahasaIndonesia','category'))
    	->where("lbs.type = '2' and lbs.parentId=0 " )
    	->where("lbs.IdLandscape = ?",$IdLandscape);
    ...
    public function getInfoMinGrade($idLandscape,$idSubject,$major){
