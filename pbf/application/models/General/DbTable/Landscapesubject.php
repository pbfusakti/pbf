...
class App_Model_General_DbTable_Landscapesubject extends Zend_Db_Table_Abstract
{
	...
	public function updateData($data,$id) {
		$id=$this->update($data,'idLandscapeSub='.$id);
		return $id;
	}
	...
	public function getLandscapeCourse($idLandscape){
    	  ...
    	  $lstrSelect = $db->select()
		 				 ...
		 				 ->join(array("s"=>"tbl_subjectmaster"),'s.IdSubject=ls.IdSubject ',array('s.IdSubject','s.BahasaIndonesia','s.SubCode','s.ShortName','CreditHours','key'=>'s.IdSubject','value'=>'CONCAT(ShortName," ",s.BahasaIndonesia)'))
							->join(array('def'=>'tbl_definationms'),'ls.SubjectType=def.IdDefinition',array('subjecttype'=>'def.BahasaIndonesia'))
							->joinLeft(array('mj'=>'tbl_programmajoring'),'mj.IDProgramMajoring=ls.IDProgramMajoring',array('Major'=>'BahasaDescription'))
    ...
    public function getLandscapeCoursePrint($idLandscape){
    	 
    	$db = Zend_Db_Table::getDefaultAdapter();
    	 
    	$lstrSelect = $db->select()
    	->from(array("ls"=>$this->_name))
    	->join(array("s"=>"tbl_subjectmaster"),'s.IdSubject=ls.IdSubject ',array('s.IdSubject','s.SubjectName','s.BahasaIndonesia','s.SubCode','s.ShortName','CreditHours','key'=>'s.IdSubject','value'=>'CONCAT(ShortName," ",s.BahasaIndonesia)'))
    	->join(array('def'=>'tbl_definationms'),'ls.SubjectType=def.IdDefinition',array('subjecttype'=>'def.BahasaIndonesia'))
    	->joinLeft(array('mj'=>'tbl_programmajoring'),'mj.IDProgramMajoring=ls.IDProgramMajoring',array('Major'=>'BahasaDescription'))
    	->where("ls.IdLandscape = ?",$idLandscape)
    	->group("ls.IdSubject")
	...
	public function getLandscapeCourseList($idLandscape,$idSemester,$semester_level,$idProgramMajoring=0,$offer=null){
    	  ...
    	  $lstrSelect = $db->select()
		 				 ...
		 				 ->group("s.IdSubject")
		 				 ->order("s.BahasaIndonesia");
		  if ($offer=='1' ) $lstrSelect->where("ls.IdSubject IN (?)",$select_course_offer);
		  if(isset($idProgramMajoring)){
		  		$lstrSelect->where("ls.IDProgramMajoring ='0' or ls.IDProgramMajoring = ?",$idProgramMajoring);
		  }
	...
	public function getMultiLandscapeCourse($Landscapes,$search=""){
    	  ...
    	  $lstrSelect = $db->select()
    	  				->distinct()
		 				 ->from(array("ls"=>$this->_name),array('ls.IdLandscape'))		
    ...
    public function getInfoMinGrade($idLandscape,$idSubject,$major=0){
    	...
    	->where("ls.IdLandscape = ?",$idLandscape)
    	->where("ls.IdSubject = ?",$idSubject)
    	->where("ls.IDProgramMajoring = 0 or ls.IDProgramMajoring=?",$major);
    		
    	$row = $db->fetchRow($select);
    	return $row;
    }
    
    public function getActiveLandscapeSubjects($IdProgram) {
    	$lobjDbAdpt = Zend_Db_Table::getDefaultAdapter();
    
    	$select = $lobjDbAdpt->select()
    	->distinct()
    	->from(array('ls'=>$this->_name),array())
    	->join(array('lan' => 'tbl_landscape'),'ls.IdLandscape=lan.IdLandscape',array('lan.IdLandscape','lan.Active','lan.ProgramDescription'))
    	->join(array("s"=>"tbl_subjectmaster"),'s.IdSubject=ls.IdSubject ',array('IdSubject','BahasaIndonesia','SubCode','CreditHours'))
    	->where('lan.IdProgram = ?',$IdProgram)
    	->where('lan.Active = ?',1)
    	->order("BahasaIndonesia");
     // echo $select;exit;
