...
class App_Model_General_DbTable_Staffmaster extends Zend_Db_Table_Abstract
{
	...
	public function fngetStaffDetails() { //Function to get the user details
     ...
     public function fnGetStaffGrade() {
     	...
     	->where('b.defTypeDesc = "Staff Grade"')
     	->where('a.Status = 1')
     	->where('b.Active = 1')
     	->order("b.defTypeDesc");
	...
	public function fngetUserStaffDetails($IdCollege) { //Function to get the user details
     ...
     public function fnGetStaffType() {
     	$lobjDbAdpt = Zend_Db_Table::getDefaultAdapter();
     	$select = $lobjDbAdpt->select()
     	->from(array('a' => 'tbl_definationms'),array('key'=>'idDefinition','value'=>'BahasaIndonesia'))
     	->join(array('b' => 'tbl_definationtypems'),'a.idDefType = b.idDefType',array('b.idDefType'))
     	->where('b.defTypeDesc = "Staff Type"')
     	->where('a.Status = 1')
     	->where('b.Active = 1')
     	->order("b.defTypeDesc");
     	$result = $lobjDbAdpt->fetchAll($select);
     ...
     public function fnGetStaffStatus() {
     	$lobjDbAdpt = Zend_Db_Table::getDefaultAdapter();
     	$select = $lobjDbAdpt->select()
     	->from(array('a' => 'tbl_definationms'),array('key'=>'idDefinition','value'=>'BahasaIndonesia'))
     	->join(array('b' => 'tbl_definationtypems'),'a.idDefType = b.idDefType',array('b.idDefType'))
     	->where('b.defTypeDesc = "Staff Status"')
     	->where('a.Status = 1')
     	->where('b.Active = 1')
     	->order("b.defTypeDesc");
     	$result = $lobjDbAdpt->fetchAll($select);
     ...
     public function fnGetStaffEdu() {
     	$lobjDbAdpt = Zend_Db_Table::getDefaultAdapter();
     	$select = $lobjDbAdpt->select()
     	->from(array('a' => 'tbl_definationms'),array('key'=>'idDefinition','value'=>'BahasaIndonesia'))
     	->join(array('b' => 'tbl_definationtypems'),'a.idDefType = b.idDefType',array('b.idDefType'))
     	->where('b.defTypeDesc = "Education Level"')
     	->where('a.Status = 1')
     	->where('b.Active = 1')
     	->order("b.defTypeDesc");
     	$result = $lobjDbAdpt->fetchAll($select);
...
public function fnupdateStafSubject($larrformData,$lintIdStaff) { //Function for updating the user
	...
	public function getListAcademicStaff($idCollege=null) { 
     ...
     public function getAcademicStaff($idCollege=null,$nama=null) { 
       $db = Zend_Db_Table::getDefaultAdapter();
	   ...
	   $select = $db->select()
	 	...
	 	if($nama){
	 		$select->where("sm.FullName like '%".$nama."%'");
	 	}
	 	//echo $select;exit;
		$row = $db->fetchAll($select);
