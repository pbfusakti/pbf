...
class App_Model_General_DbTable_Studentregistration extends Zend_Db_Table_Abstract {
...
//	}
    ...
    public function fetchStudentHistoryDetails($lintidreg) {
        ...
        $sql = $db->select()
                        ->from(array('a' => 'tbl_studentregistration'), array('a.*'))
                        ->joinleft(array('p'=>'student_profile'),'p.appl_id=a.IdApplication',array('appl_fname','appl_mname','appl_lname','appl_id','appl_phone_hp'))
                        ...
                        ->joinLeft(array('g' => 'tbl_user'), 'g.iduser=a.UpdUser', array("g.loginName","g.iduser"))
                        ->joinLeft(array('h' => 'tbl_definationms'), 'a.profileStatus = h.IdDefinition', array('h.DefinitionDesc'))
                        ->where('a.IdStudentRegistration = ?', $lintidreg);
        $result = $db->fetchRow($sql);
        return $result;
    }
...
//			return $result;
        ...
        $lstrSelect = $this->lobjDbAdpt->select()
                        ...
                        ->join(array('sm'=>'tbl_semestermaster'),'a.IdSemesterMain=sm.IdSemesterMaster')
                        ->joinLeft(array("b" => "tbl_definationms"), "a.profileStatus = b.idDefinition", array("b.DefinitionDesc",'b.BahasaIndonesia','pdpt_code'))
    ...
    public function fetchStudentProfileSemester($lintidreg,$semester) {
    ...
    public function fetchStudentSemesterHistory($lintidreg) {
        ...
        $sql = $db->select()
                        ...
                        ->join(array('e' => 'tbl_semestermaster'), 'e.IdSemesterMaster=a.IdSemesterMain', array('e.SemesterMainName', 'e.SemesterMainCode','e.SemesterCountType'))
                        ->joinLeft(array('c' => 'tbl_user'), 'c.iduser=a.UpdUser', array("c.loginName"))
                        ->joinLeft(array('deftn' => 'tbl_definationms'), 'deftn.idDefinition=a.studentsemesterstatus', array('deftn.DefinitionCode','BahasaIndonesia','pdpt_code'))
    ...
    public function getStudentforcredtTransfer($data) {
        $lstrSelect = $this->lobjDbAdpt->select()
                        ->from(array('a' => 'tbl_studentregistration'), array('key' => 'a.IdStudentRegistration'))
                        ->joinLeft(array('b' => 'applicant_profile'),"a.IdApplication=b.appl_id", array('name' => "CONCAT_WS(' ',IFNULL(b.appl_fname,''),IFNULL(b.appl_mname,''),IFNULL(b.appl_lname,''))"))
    ...
    public function getStudentRegistrationDetail($id) {
        $sql = $this->lobjDbAdpt->select()
                        ->from(array('a' => 'tbl_studentregistration'), array('a.registrationId', 'a.IdProgram', "CONCAT_WS(' ',IFNULL(a.FName,''),IFNULL(a.MName,''),IFNULL(a.LName,'')) AS name", "a.ExtraIdField1 AS NRIC", "a.IdProgram", "a.IdIntake", "a.profileStatus", 'a.IdStudentRegistration', 'a.Gender', 'a.HomePhone', 'a.CellPhone','idTranscriptProfile','IdLandscape','IdProgramMajoring'))
                        ->joinLeft(array('b' => 'tbl_program'), "a.IdProgram = b.IdProgram", array("b.ProgramName", "b.IdScheme"))
    ...
    public function getStudentRegistrationDetailbyProgram($idprogram,$idintake) {
    	$sql = $this->lobjDbAdpt->select()
    	->from(array('a'=>'tbl_studentregistration'))
    	->join(array('ap'=>'applicant_profile'),'a.idapplication=ap.appl_id',array('ap.*','StudentName'=>"CONCAT_WS(' ',IFNULL(ap.appl_fname,''),IFNULL(ap.appl_mname,''),IFNULL(ap.appl_lname,''))") )
    	->joinLeft(array('b' => 'tbl_program'), "a.IdProgram = b.IdProgram", array("b.ProgramName", "b.IdScheme"))
    	->joinLeft(array('c' => 'tbl_intake'), "a.IdIntake = c.IdIntake", array("c.IntakeDesc"))
    	->joinLeft(array('d' => 'tbl_definationms'), "a.profileStatus = d.idDefinition", array("d.DefinitionDesc"))
    	->joinLeft(array('f' => 'tbl_definationms'), "a.SpecialTreatmentType = f.idDefinition", array("f.DefinitionDesc as specialtreatment"))
    ...
    public function fngetstudents($larrformData) {
		$sql = $this->lobjDbAdpt->select()
		->from(array('c' => 'tbl_studentregistration'), array('c.registrationId', 'c.IdApplication','c.FName','c.MName','c.LName','c.ApplicationDate','c.ExtraIdField1','c.Gender','c.CorrsAddressDetails','c.CorrsZip','c.RelativeName'))
    ...
    public function getStudentByRegId($registrationId,$program) {
    	$db = Zend_Db_Table::getDefaultAdapter();
    	$sql = $db->select()
    	->from(array('a' => 'tbl_studentregistration'))
    	->joinLeft(array('c' => 'tbl_program'), 'a.IdProgram = c.IdProgram', array('c.ProgramName', 'c.IdScheme'))
...
/*
	...
	public function getListStudent($post=null,$IdSemester=null) {
        ...
        $lstrSelect = $db->select()->from(array('sa' => 'tbl_studentregistration'), array('sa.*'))
                        ...
                        ->joinLeft(array('p'=>'student_profile'),'p.appl_id=sa.IdApplication',array('appl_fname','appl_mname','appl_lname','appl_religion','appl_dob','appl_birth_place'))
                        ->joinLeft(array('deftn' => 'tbl_definationms'), 'deftn.idDefinition=sa.profileStatus', array('deftn.DefinitionCode','Status')) //Application STtsu
                        ...
                        ->joinLeft(array('prg' => 'tbl_program'), 'prg.IdProgram=sa.IdProgram', array('prg.ArabicName','ProgramName','strata'))
                        ->joinLeft(array('intk' => 'tbl_intake'), 'intk.IdIntake=sa.IdIntake', array('intk.IntakeId','class_start'))
                        ->joinLeft(array('schpr'=>'tbl_program_scheme'),'schpr.IdProgram=prg.IdProgram',array())
                        ->joinLeft(array('sch' => 'tbl_scheme'), 'sch.IdScheme=schpr.IdScheme', array('schemeeng'=>'sch.EnglishDescription','scheme'=>'sch.ArabicDescription','jenis'=>'sch.Jenis','jeniseng'=>'sch.TypeOfEducation'))
                        ->joinLeft(array('ls'=>'tbl_landscape'),'ls.IdLandscape=sa.IdLandscape',array('landscape'=>'ProgramDescription'))
                        ...
                        ->joinLeft(array('mj'=>'tbl_programmajoring'),'mj.IDProgramMajoring=sa.IdProgramMajoring',array('EnglishDescription','BahasaDescription'))
                        ->joinLeft(array('c'=>'tbl_collegemaster'),'c.IdCollege=prg.IdCollege',array('NamaKolej'=>'ArabicName','c.IdCollege','CollegeName'))
        ...
        if (isset($post['IdMajoring']) && !empty($post['IdMajoring'])) {
        	$lstrSelect->where("sa.IdProgramMajoring = ?",$post['IdMajoring']);
        }
        ...
        if (isset($post['IdStudentRegistration']) && !empty($post['IdStudentRegistration'])) {
        	$lstrSelect->where("sa.IdStudentRegistration = '". $post['IdStudentRegistration']."'");
        ...
        if (isset($post['start_nim']) && !empty($post['end_nim'])) {          	
        	   $lstrSelect->where("sa.registrationId >= '". $post['start_nim']."' AND sa.registrationid <= '".$post['end_nim']."'");
        }
        
        
        if (isset($post['yudisium_status']) && $post['yudisium_status']=='1') {
        	$lstrSelect->join(array('pregrad'=>'pregraduate_list'),'sa.idstudentregistration=pregrad.idstudentregistration');
        	
        }
        
        if (isset($post['yudisium_status']) && $post['yudisium_status']=='2') {
        	$lstrSelect->join(array('grad'=>'graduate'),'sa.idstudentregistration=grad.idstudentregistration');
        ...
        if (isset($post['IdSemesterGraduation']) && $post['IdSemesterGraduation']!='') {
        	$lstrSelect->join(array('pregrad'=>'pregraduate_list'),'sa.idstudentregistration=pregrad.idstudentregistration')
        	           ->join(array('skr'=>'graduation_skr'),'skr.id=pregrad.dean_approval_skr')
        				->where('skr.IdSemesterMain=?',$post['IdSemesterGraduation']);
    ...
    public function SearchStudent($data=null){
    	...
    	$db = Zend_Db_Table::getDefaultAdapter();
                        	...
                        	$sql = $db->select()
                        	->from(array('sr' => 'tbl_studentregistration'),array('IdBranch','registrationId','IdStudentRegistration','IdProgram','IdLandscape','IdProgramMajoring','IdIntake'))
                        	->joinLeft(array('m'=>'tbl_programmajoring'),'sr.IdProgramMajoring=m.IDProgramMajoring',array('Majoring'=>'BahasaDescription'))
                        	...
                        	if(isset($data)){
                        		...
                        		if (isset($data['Idstudentreg']) && $data['Idstudentreg']!=''){
                        			$sql->where('sr.idstudentregistration = ?', $data['Idstudentreg']);
                        		}
    ...
    public function SearchStudentNoSem($data=null){
    	...
    	if ($session->IdRole == 3) {
    		...
    		//->joinLeft(array('sss'=>'tbl_studentsemesterstatus'),'sss.IdStudentRegistration=sr.IdStudentRegistration',array('idstudentsemsterstatus','Level'))
    		->join(array('sp'=>'student_profile'),'sp.appl_id=sr.IdApplication',array('student_name'=>"CONCAT_WS(' ',appl_fname,appl_mname,appl_lname)"))
    		//->joinLeft(array('d' => 'tbl_definationms'), "d.idDefinition=sss.studentsemesterstatus", array('studentsemesterstatus'=>"DefinitionDesc"))
    		->where('sr.Status=198')
    		->order('sr.registrationId');
    		$id=$session->IdStaff;
    		$sql->where('sr.AcademicAdvisor = ?', $id);
    		if(isset($data)){
    ...
    public function getPopulation($idsemester,$strata,$intake){
    	$db = Zend_Db_Table::getDefaultAdapter();
    	$select = $db->select()
    	->from(array('sr'=>$this->_name),array('count'=>'count(distinct idStudentRegistration)') )
    	->join(array('str'=>'tbl_studentregsubjects'),'sr.IdStudentRegistration=sr.IdStudentRegistration',array())
    	->join(array('pr'=>'tbl_program'),'sr.IdProgram=pr.IdProgram',array('ProgramCode','ProgramName'=>'ArabicName'))
    	->where('str.IdSemesterMain = ?',$idsemester)
    	->order('count desc')
    	->group('pr.IdProgram');
    ...
    public function idInStudentProfile($applid) {
    	$db = Zend_Db_Table::getDefaultAdapter();
    	$select = $db->select()
