...
class App_Model_General_DbTable_TranscriptProfile extends Zend_Db_Table { //Model Class for Users Details
	...
	public function getTranscriptProfileAll(){
		...
		->joinLeft(array("mj"=>"tbl_programmajoring"),'mj.IdProgramMajoring=tr.IdMajoring',array('BahasaDescription'));
	...
	public function getTranscriptGroup() {
		
		$lobjDbAdpt = Zend_Db_Table::getDefaultAdapter();
		$select = $lobjDbAdpt->select()
		->from(array('tp'=>'transcript_profile'),array())
		->join(array('pr'=>'tbl_program'),'tp.IdProgram=pr.IdProgram',array())
		->join(array('grp'=>'transcript_profile_grp'),'tp.IdTranscriptProfile=grp.IdTranscriptProfile',array())
		->join(array('a' => 'tbl_definationms'),'a.IdDefinition=grp.idgroup',array('a.idDefinition','a.BahasaIndonesia','a.DefinitionCode'))
		->join(array('b' => 'tbl_definationtypems'),'a.idDefType = b.idDefType',array('b.idDefType'))
		->where('b.defTypeDesc = "Transcript Profile"')
		->where('a.Status = 1')
		->where('b.Active = 1')
		->group('a.idDefinition')
		->order("b.defTypeDesc");
