...
class Examination_Form_ExamScript extends Zend_Form
{
	...
	public function init()
	{
		...
		if ($this->script_id!='') {
			...
			if ($contri) {
				...
				foreach ($contri as $value) {
					 $tappend .= '<tr>'.
		    		  '<td>'.
		    		  '<input type="hidden" name="contributors[]" value="'.$value['IdStaff'].'" />'.
		    		  $value['FullName'] .
		    		  '</td><td>'.
		    		  '<input type="hidden" name="contributorspos[]" value="'.$value['position'].'" />'.
		    		  $value['Position'] .
		    		  '</td>'.
		...
		//program
		$table = '<table id="contributor" class="table" width="500px"><thead><tr><th>Contributor Name</th><th>Contributor Position</th><th width="30px">Action</th></tr></thead><tbody>'.$tappend.'</tbody><tfoot><tr><td>&nbsp;</td><td>&nbsp;</td></tr></tfoot></table>';
		$this->addElement('hidden', 'contributor_list', array(
		    'label' => 'Contributor',
		    'ignore' => true,
		    'description' => '<a href="#" class="btn" id="assign-contributor">Add Contributor</a><br />'.$table
		));
